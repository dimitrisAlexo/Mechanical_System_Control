function [] = redesign_1(x0)
    ...
    plot(t, xd(t));
    title('Redesign1: Trajectory Tracking')
    xlabel('{t} (sec)')
    ylabel('{x_1} (m)')
    ...
    function dxdt = odefun(t,x)
        ...
        control = u_approx*m_approx*g*sat(w*x(2), 1) ...
            + (k_approx + Dk_approx*(1-exp(-b_approx*t)))*x(1)*(1+a_approx*(x(1))^2) + m*xddd(t) - transpose(F)*e ...
            - (0.9375*g*abs(sat(w*x(2), 1)) + 3.375*abs(x(1)) + 4.4375*(abs(x(1)))^3 + 0.75*abs(xddd(t))) ...
            *sat(transpose(b)*P*e, 1);
